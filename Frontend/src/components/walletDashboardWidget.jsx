...
const dashboardWidget = ({ linkedWalletAddress, embeddedWalletAddress, walletBalance, ready, selected }) => {
  ...
  useEffect(() => {
    ...
    fetch("https://api.coingecko.com/api/v3/simple/price?ids=binancecoin&vs_currencies=usd")
      ...
      .then((data) => {
        ...
        console.log(`The price of BNB is $${bnbPrice}`);
      })
      .catch((error) => console.error("Error fetching BNB price data:", error));
  ...
  //Use Effect to Load BNB Balance Through Privy Wallet
  useEffect(() => {
    ...
    if (linkedWalletAddress) {
      ...
      const url = `https://api.bscscan.com/api?module=account&action=balance&address=${linkedWalletAddress}&apikey=${apiKey}`;
      console.log("URL:", url);
      ...
      fetch(url)
        ...
        .then((data) => {
          ...
          console.log(`The User's BNB balance is: ${bnbBalance}`);
        })
        .catch((error) => console.error("Error fetching User BNB Account Balance data:", error));
    }
    ...
    if (embeddedWalletAddress && !linkedWalletAddress) {
      ...
      const url = `https://api.bscscan.com/api?module=account&action=balance&address=${embeddedWalletAddress}&apikey=${apiKey}`;
      console.log("URL:", url);
